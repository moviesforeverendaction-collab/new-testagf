...
TELEGRAM_MAX_CHUNK = 1024 * 1024  # Telegram hard ceiling — 1 MB


class Telegram:
    # ── Bot 1 — commands + download MTProto ──────────────────────────────
    BOT_TOKEN = str(env.get("BOT_TOKEN", ""))
    API_ID    = int(env.get("API_ID",    0))
    API_HASH  = str(env.get("API_HASH",  ""))

    # ── Bot 2 — stream MTProto (separate bot token, same or different app) ─
    # Use a second bot token from @BotFather. API_ID_2/API_HASH_2 can be the
    # same as API_ID/API_HASH — Telegram allows multiple bots per app.
    BOT_TOKEN_2 = str(env.get("BOT_TOKEN_2", ""))
    API_ID_2    = int(env.get("API_ID_2",   0)) or int(env.get("API_ID", 0))
    API_HASH_2  = str(env.get("API_HASH_2", "")) or str(env.get("API_HASH", ""))
...
class Server:
    ...
    STREAM_CHUNK_SIZE      = TELEGRAM_MAX_CHUNK
